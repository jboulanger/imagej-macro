...
function process_list_of_files(tablename, condition, channel_names, roi_channel, obj_channel, mask_channel, downsampling_factor,keys,vals) {
	...
	for (n = 0; n < N; n++) {
		...
		process_image(filename, condition, channel_names, roi_channel, obj_channel, mask_channel, downsampling_factor, all_keys, all_vals);		
		close();
		print("\n");
...
function process_single_file(filename, condition, channel_names, roi_channel, obj_channel, mask_channel, downsampling_factor, keys, vals) {
	/* 
...
function generateTestImage() {
	...
	c = 3;
	n = 2;
	newImage("HyperStack", "32-bit composite-mode", w, h, c, 1, 1);
	run("Add Noise");
	run("Gaussian Blur...", "sigma=2 stack");
	setThreshold(0.75*getValue("Mean")+0.25*getValue("Max"), getValue("Max"));
	run("Convert to Mask","stack");
	Stack.setChannel(0); run("Cut");
	Stack.setDisplayMode("composite");
	setColor(255);
	for (j=0;j<n;j++) {
		for (i=0;i<n;i++) {
			makeOval((i+0.75+random/10) / (n+1)*w, (j+0.75+random/10) / (n+1)*h, w/8, h/8);
			run("Fill", "slice");	
		}
	}
	run("Select None");
...
function measureColoc(id, dist, rois, condition_name, channel_names, image_name, keys, vals) {
