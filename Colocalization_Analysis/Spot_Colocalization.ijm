...
function detectSpotsDOG(scale, logpfa) {
	...
	for (c = 1; c <= channels; c++) {
		Stack.setChannel(c);
		run("Select None");
		t = getValue("Mean")+2*getValue("StdDev");
		run("Macro...", "code=v=255*(v<"+t+") slice");	
	}	
	setThreshold(128, 255);
	run("Convert to Mask", "method=Default background=Default light");
	run("Watershed","stack");
	...
	for (c = 1; c <= channels; c++) {
		...
		run("Select None");
		n0 = roiManager("count");
		run("Analyze Particles...", "size="+(PI*scale*scale)+"-Infinity pixel add slice");
		n1 = roiManager("count");
