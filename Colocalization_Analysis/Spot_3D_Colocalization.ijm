// @File(label="Input",description="Use image to run on current image or test to run on a generated test image") path
...
// @Double(label="Specificity A [log10]", value=1, style=slider, min=0, max=5) pfaa
// @Double(label="Spot size B [um]", value=0.2) spot_sizeb
// @Double(label="Specificity B [log10]", value=1, style=slider, min=0, max=5) pfab
...
if (mode==1 || mode==2) {
	Overlay.remove();
	setColor("red");
	coords2Overlay(A,1,channela);
	setColor("green");
	coords2Overlay(B,2,channelb);
}
...
function coords2Overlay(coords,r,channel) {
	...
	for (i = 0; i < coords.length / 3; i++) {
		...
		Overlay.setPosition(channel, z, 0);
		Overlay.add;
	}
	Overlay.show();
}
...
function detect3DSpots(channel, size, pfa) {
	...
	id3 = getImageID;	
	run("Maximum 3D...", "x="+p+" y="+p+" z="+2*p);
	imageCalculator("Subtract 32-bit stack",id3,id1);
	setThreshold(-0.0001, 0.0001);
	...
	for (slice = 1; slice <= slices; slice++) {
		...
		if (Roi.size != 0) {
			...
			for (i = 0; i < xpoints.length; i++) {
				coords[3*j] = xpoints[i]*dx;
				coords[3*j+1] = ypoints[i]*dy;
				coords[3*j+2] = slice*dz;
				j = j + 1;				
			}
		}
	}
